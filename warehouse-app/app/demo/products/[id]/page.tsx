...
export default function DemoProductDetail({ params }: { params: { id: string } }) {
  ...
  if (!product || !form) return <div className="p-8 text-gray-400">Тауар табылмады</div>;

  function set(k: string, v: string) { setForm((f) => f ? { ...f, [k]: v } : f); }

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form || !form.name || !form.sku || !form.categoryId || !form.unitId) { setError("Міндетті өрістерді толтырыңыз"); return; }
    ...
    dispatch({ type: "NOTIFY", message: "Тауар сақталды", kind: "success" });
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <button onClick={() => router.back()} className="text-xs text-blue-600 hover:underline mb-1 block">← Тауарларға қайту</button>
        <h1 className="text-xl font-semibold">{product.name}</h1>
      </div>

      {stock.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <p className="text-sm font-medium text-gray-700 mb-2">Ағымдағы қалдықтар</p>
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-lg p-6 space-y-4">
        ...
        <div className="flex gap-3">
          <button type="submit" className="px-5 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">Сақтау</button>
