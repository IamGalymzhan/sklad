...
export default function DemoLocations() {
  ...
  function create(e: React.FormEvent) {
    ...
    dispatch({ type: "NOTIFY", message: "Сақтау орны құрылды", kind: "success" });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-semibold">Сақтау орындары</h1><p className="text-sm text-gray-500">Қоймалардағы аймақтар, сөрелер мен ұяшықтар</p></div>
        <button onClick={() => setOpen(!open)} className="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700">+ Орын құру</button>
      </div>

      {open && (
        <form onSubmit={create} className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex gap-3 items-end flex-wrap max-w-2xl">
          <div className="w-48">
            <label className="text-xs font-medium text-gray-700 block mb-1">Қойма *</label>
          ...
          <div className="w-28"><label className="text-xs font-medium text-gray-700 block mb-1">Коды (A-01) *</label><input value={f.code} onChange={(e) => setF((x) => ({ ...x, code: e.target.value }))} className={inp} required /></div>
          <div className="flex-1"><label className="text-xs font-medium text-gray-700 block mb-1">Атауы *</label><input value={f.name} onChange={(e) => setF((x) => ({ ...x, name: e.target.value }))} className={inp} required /></div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-50"><tr><th className="px-4 py-3 text-left font-medium text-gray-500">Коды</th><th className="px-4 py-3 text-left font-medium text-gray-500">Атауы</th><th className="px-4 py-3 text-left font-medium text-gray-500">Қойма</th></tr></thead>
