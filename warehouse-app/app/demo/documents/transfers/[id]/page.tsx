...
export default function DemoTransferDetail({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <button onClick={() => router.back()} className="text-xs text-blue-600 hover:underline mb-1 block">← Ауыстыруларға қайту</button>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-5">
        ...
        <dl className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
          <div><dt className="text-gray-500">Қайдан</dt><dd className="font-medium">{fromWh?.name ?? "—"}</dd></div>
          <div><dt className="text-gray-500">Қайда</dt><dd className="font-medium">{toWh?.name ?? "—"}</dd></div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        ...
        <table className="min-w-full divide-y divide-gray-100 text-sm">
          <thead><tr>
            ...
            <th className="px-4 py-2 text-left font-medium text-gray-500">Қайдан (орын)</th>
            <th className="px-4 py-2 text-left font-medium text-gray-500">Қайда (орын)</th>
        ...
        <div className={`border rounded-lg p-4 flex items-center justify-between ${canConfirm ? "bg-yellow-50 border-yellow-200" : "bg-red-50 border-red-200"}`}>
          <div>
            <p className={`text-sm font-medium ${canConfirm ? "text-yellow-800" : "text-red-700"}`}>
              {canConfirm ? "Жоба растауға дайын" : "Ауыстыру үшін қалдықтар жеткіліксіз"}
            </p>
            <p className={`text-xs mt-0.5 ${canConfirm ? "text-yellow-600" : "text-red-500"}`}>
              {canConfirm ? "Тауарларды ауыстыру үшін «Растау» түймесін басыңыз" : "Жөнелтуші қоймадағы қорларды толықтырыңыз"}
            </p>
          </div>
          {canConfirm && (
      ...
      {doc.status === "CONFIRMED" && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <p className="text-sm text-green-700">
            Құжат расталды. Тауарлар «{fromWh?.name}» қоймасынан «{toWh?.name}» қоймасына ауыстырылды.
          </p>
        </div>
      )}
    </div>
