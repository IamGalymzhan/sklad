...
export default function DemoIssueNew() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!warehouseId || !recipientName.trim()) { setError("Қойма мен алушыны толтырыңыз"); return; }
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <h1 className="text-xl font-semibold">Жаңа шығыс</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <F label="Алушы *"><input value={recipientName} onChange={(e) => setRecipientName(e.target.value)} className={inp} required placeholder="Бөлім, қызметкер немесе ұйым" /></F>
                ...
                <div key={i} className="space-y-1">
                  ...
                  {available !== null && (
                    <p className={`text-xs ml-0 ${insufficient ? "text-red-600" : "text-gray-500"}`}>
                      {insufficient ? `⚠ Жеткіліксіз: қол жетімді ${available}` : `Қол жетімді: ${available}`}
                    </p>
                  )}
                </div>
              );
