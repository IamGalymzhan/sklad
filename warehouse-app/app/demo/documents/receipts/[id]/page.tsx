...
export default function DemoReceiptDetail({ params }: { params: { id: string } }) {
  ...
  function confirm() {
    ...
    dispatch({ type: "NOTIFY", message: `${doc!.documentNumber} құжаты расталды`, kind: "success" });
  }

  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <button onClick={() => router.back()} className="text-xs text-blue-600 hover:underline mb-1 block">← Кірістерге қайту</button>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-5">
        ...
        <dl className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
          <div><dt className="text-gray-500">Жеткізуші</dt><dd className="font-medium">{supplier?.name ?? "—"}</dd></div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        ...
        <table className="min-w-full divide-y divide-gray-100 text-sm">
          <thead><tr>
            ...
            <th className="px-4 py-2 text-right font-medium text-gray-500">Бағасы</th>
            <th className="px-4 py-2 text-right font-medium text-gray-500">Сомасы</th>
          </tr></thead>
            ...
            {doc.items.map((item) => {
              ...
              return (
                <tr key={item.id}>
                  ...
                  <td className="px-4 py-2.5 text-right text-gray-600">{item.unitPrice ? `${item.unitPrice.toLocaleString("kk-KZ")} ₸` : "—"}</td>
                  <td className="px-4 py-2.5 text-right font-medium">{item.unitPrice ? `${(item.quantity * item.unitPrice).toLocaleString("kk-KZ")} ₸` : "—"}</td>
                </tr>
              );
            })}
          </tbody>
          {total > 0 && (
            <tfoot>
              <tr className="border-t border-gray-200">
                <td colSpan={4} className="px-4 py-2.5 text-right font-semibold text-gray-700">Барлығы:</td>
                <td className="px-4 py-2.5 text-right font-bold text-gray-900">{total.toLocaleString("kk-KZ")} ₸</td>
              </tr>
            </tfoot>
          )}
        </table>
      ...
      {doc.status === "DRAFT" && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-yellow-800">Жоба расталмаған</p>
            <p className="text-xs text-yellow-600 mt-0.5">Тауарларды қоймаға кіріске алу үшін «Растау» түймесін басыңыз</p>
          </div>
      ...
      {doc.status === "CONFIRMED" && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <p className="text-sm text-green-700">Құжат расталды. Тауарлар қоймаға кіріске алынды.</p>
        </div>
      )}
    </div>
  );
